...
import { ThemeDeclaration, Theme } from './type';
...
export const makeTheme = (declaration: ThemeDeclaration): Theme => {
    const theme = cloneDeep(declaration);

    // move spacing to a function
    if ('spacing' in theme) {
    ...
    return deepFreeze(theme) as Theme;
};
